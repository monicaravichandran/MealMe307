        ...
        ref.child("users").observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            if let dict = snapshot.value as? [String:Any?] {
                for item in dict {
                    if item.key == key {
                        ...
                        if let val = item.value as? NSDictionary{
                            let curUser = MealMeUser(name: val["name"] as? String ?? "", userEmail: val["email"] as? String ?? "", zip: val["zip"] as? Int ?? 0, phone: val["phone"] as? String ?? "", meals: val["meals"] as?  [String] ?? [], reviews: val["reviews"] as?  [String] ?? [], totalRating: val["totalRating"] as? Float ?? 0.0, numReviews: val["numReviews"] as? Int ?? 0)
                            mealMeUser = curUser
                        }
                    }
                ...
                if(exists == 1){
                    ...
                    let nilUser = MealMeUser(name: "NilUser", userEmail: "NIL", zip: 0, phone: "NIL", meals: [], reviews: [], totalRating: 0.0, numReviews: 0)
                    completion(nilUser)
                }
            }
